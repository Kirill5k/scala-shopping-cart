  ...
  "An AuthController" should {
    ...
    "POST /users/auth/login" should {
      "return token on success" in {
        ...
        when(authServiceMock.login(any[User.Name], any[Password])).thenReturn(IO.pure(JwtToken("token")))
        ...
        verify(authServiceMock).login(User.Name("boris"), Password("password"))
      }
      ...
      "return forbidden when password is incorrect" in {
        ...
        when(authServiceMock.login(any[User.Name], any[Password]))
      ...
      "return bad request when empty login or password" in {
        ...
        verify(authServiceMock, never).login(any[User.Name], any[Password])
      }
    ...
    "POST /users" should {
      "register new user on success" in {
        ...
        when(authServiceMock.create(any[Username], any[Password])).thenReturn(IO.pure(User.Id(userId)))
